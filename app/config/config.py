...
class Config:
    ...
    def __init__(self,*,R1,R2,ipAdress,scanInterval,userBaseII,userTxGain,scanPerControl,NFFT,\
                 timeout,processDetection,processDoppler,plotSaveRaw,plotSaveDetection,plotSaveDoppler,processSaveDetection= False,\
                    camOn,cameraSave,camSaveQuality,postProcessing=-1,scanResolutionBins=32,**kwargs):
        ...
        self.R1=R1
        self.R2=R2
        self.NFFT=NFFT # number of scans per process
        ...
        self.postProcessing=postProcessing # this is the index of the file going to be processed, -1 means no processing

        self.plotSaveRaw=plotSaveRaw
        self.plotSaveDetection=plotSaveDetection
        self.plotSaveDoppler=plotSaveDoppler
        self.processSaveDetection=processSaveDetection

        #cam
        self.camOn=camOn
        self.cameraSave=cameraSave
        self.camSaveQuality=camSaveQuality
        ...
        # self.plotDetection=plotDetection if self.processDetection == 1 else 0
        # self.plotVelocity=plotVelocity if self.processVelocity == 1 else 0   
        self.plot = [plot for plot in [self.plotSaveRaw ,self.plotSaveDetection ,self.plotSaveDoppler] if plot == DataMode.PLOT]
        self.plot = len(self.plot)
