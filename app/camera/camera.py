...
class Camera:
    ...
    def initialize(cls,camRec,myCsv : MyCsv,stopRec,radSend,config):
        ...
        if not Camera.initCameraConfig() and cls.config.postProcessing == -1:            
            stopRec.send(1)
            return
        
        if cls.config.postProcessing == -1:
            try:
                ...
                #stops the system in case cam cannot be initialized
                print(e)
                Logger.error(e)
                stopRec.send(1)
                return
        ...
        # camera images reconstitution 
        else:
            image_folder=f"outputFolders/dataAcquisition_{cls.config.postProcessing}/images/"
            frames=[]
            # Get all image files in the folder
            images = glob.glob(f"{image_folder}/*.jpeg")
            images.sort(key=lambda x:float(x[len(image_folder):-4]))  # Sort the images by name    
                
            for image in images:
                if cls.stopRec.poll():                    
                    return
                frames.append([cv2.imread(image),float(image[len(image_folder):-4])])        
                
            i=0
            radSend.send(1)
            while(i< len(frames)):
                
                frame=cv2.resize(frames[i][0],(1000,600))
